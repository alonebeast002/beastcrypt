...
# FIX: Katana timeout now respects env var, default raised to 300s
KATANA_TIMEOUT = int(os.environ.get("KATANA_TIMEOUT", "300"))

# FIX: CDX limits raised for large targets
...
def _strip_domain_tld(domain):
    """
    Strip scheme + www prefix + TLD (.com, .org, etc.) from domain.
    e.g. https://www.example.com → example
         sub.example.co.uk      → sub_example
    """
    clean = domain.split("://")[-1].rstrip("/").lower()
    clean = re.sub(r'^www\.', '', clean)
    # remove TLD(s): last dot-segment(s) like .com, .co.uk
    clean = re.sub(r'\.(com|net|org|io|co|uk|in|de|fr|jp|au|us|gov|edu|info|biz|me|app|dev|ai)(\.[a-z]{2})?$', '', clean)
    # replace remaining dots/special chars with underscore
    clean = re.sub(r'[^\w]', '_', clean)
    return clean.strip('_') or "target"

def _url_to_filename(url, ext_override=None):
    """
    Convert a URL to a clean filename preserving the path.
    e.g. https://example.com/static/app.bundle.js → static_app_bundle.js
    """
    parsed   = urllib.parse.urlparse(url)
...
def curl_get(url, timeout=25, retries=3, raw_url=None):
    ...
    cmd = [
        ...
        "--max-redirs", "5",          # FIX: cap redirect loops
        "--compressed", "--retry", "0",
        ...
        try:
            ...
            if "__HTTPCODE__" in out:
                ...
                if code in (429, 502, 503) and attempt < retries:
                    # FIX: exponential backoff on rate limit / server errors
                    wait = 2 ** attempt
...
# FIX: removed dead code_file / .httpcode creation — curl writes code to stdout directly
...
def cdx_fetch_urls(domain, exts_filter=None, limit=None):
    ...
    for api, label in queries:
        ...
        if status != 200 or not body.strip():
            ...
            sys.stdout.flush()
            # FIX: if rate limited, log clearly and continue — backoff already done in curl_get
            if status == 429:
...
def wayback_hunter(domain, exts_filter, output_dir, save_files=True, json_only=False):
    ...
    # FIX: microsecond suffix to avoid timestamp collision on same-second runs
    ...
    for i, u in enumerate(urls, 1):
        ...
        # FIX: show percentage alongside count
        pct = int(i / total * 100)
        sys.stdout.write(f"\r  {DIM}[{i:>{len(str(total))}}/{total} {pct:>3}%]{RST} {DIM}↓{RST} {orig[:tw()-25]}")
        sys.stdout.flush()
    ...
    for api, label in queries:
        ...
        if status != 200 or not body.strip():
            ...
            if status == 429:
                ...
                time.sleep(10)  # FIX: extra backoff on 429 before next CDX query
            continue
        before = len(url_ts)
        for line in body.splitlines():
            line = line.strip()
...
def fetch_katana_js(domain):
    ...
    try:
        result = subprocess.run(
            ...
            timeout=KATANA_TIMEOUT,   # FIX: configurable timeout
            errors="ignore"
        )
        seen, pairs = set(), []
...
# ─────────────────────────────────────────────────────────────────────────────
# NEW FEATURE: Structured JS + MAP download into domain-named folder tree
# Layout:
#   <output_dir>/downloads/<domain_no_tld>/js/<source_url_filename>.js
#   <output_dir>/downloads/<domain_no_tld>/maps/<source_url_filename>.js.map
# ─────────────────────────────────────────────────────────────────────────────
def download_js_and_maps_structured(domain, url_pairs, output_dir):
    """
    Downloads all JS files and their .map counterparts from Wayback into a
    clean folder structure:
        downloads/<domain>/js/<filename>.js
        downloads/<domain>/maps/<filename>.js.map

    Returns (js_records, map_records) where each record is a dict with
    keys: orig_url, snap_url, local_path, source (WB/LV/FAIL)
    """
    ...
    for i, (orig_url, snap_url) in enumerate(url_pairs, 1):
        ...
        if body and len(body.strip()) > 50:
            ...
            if ms == 200 and len(mb.strip()) > 10:
                ...
                # FIX: map_src based on used_snap flag, not snap_url existence
...
def validate_and_download_js(url_pairs, output_dir):
    ...
    for i, (orig_url, snap_url) in enumerate(deduped, 1):
        ...
        if body and len(body.strip()) > 50:
            ...
            if ms == 200 and len(mb.strip()) > 10:
                mb = _strip_wayback(mb)
                # FIX: map_src correctly uses used_snap, not snap_url existence
...
def scan_secrets(live_js, live_maps, output_dir, domain):
    ...
    all_findings = []
    # FIX: global dedup set across all files — same (type, value) won't appear twice
    global_seen = set()
...
def js_scanner_run(domain, output_dir):
    ...
    # Standard flat download (existing behaviour)
    live_js, live_maps = validate_and_download_js(all_pairs, folder)

    # NEW: Also save structured download tree
...
def parse_cli():
    p = argparse.ArgumentParser(
        description="BEAST v1.2 — Wayback Hunter + JS Secret Scanner  (by ALONE BEAST)",
        add_help=True
    )
